  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <section className="flex h-[560px]">
        <div className="w-[660px] shrink-0 flex flex-col justify-center gap-8 bg-[#0D2818] px-14 py-20">
          <span className="text-[#C9A962] font-semibold text-xs tracking-[2px]">PIONEERS IN TRAVEL TECHNOLOGY VENTURE CAPITAL</span>
          <h1 className="font-playfair text-[52px] italic font-normal text-white leading-[1.1]">Investing in the Future of&nbsp;<span className="whitespace-nowrap">Global Travel</span></h1>
        ...
        <div className="flex-1 bg-cover bg-center" style={{ backgroundImage: "url('/images/modern-hero.png')" }} />
      </section>

      {/* Stats Bar */}
      <section className="flex justify-between bg-white px-14 py-12 border-y border-[#E5E5E5]">
        {[{ val: "110+", label: "Portfolio Companies" }, { val: "150+", label: "Corporate Partners" }, { val: "~$3B", label: "Assets Under Management" }].map((s, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <span className="font-playfair text-[44px] italic font-semibold text-[#0D2818]">{s.val}</span>
      ...
      {/* Testimonials */}
      <section className="bg-white px-14 py-20 flex flex-col items-center gap-12 border-b border-[#E5E5E5]">
        <div className="flex flex-col items-center gap-3">
          <span className="text-[#00D776] font-semibold text-xs tracking-[2px]">WHAT OUR PARTNERS SAY</span>
          <h2 className="font-playfair text-4xl italic text-[#1A2E23]">Trusted by Industry Leaders</h2>
        </div>
      ...
      {/* Statement */}
      <section className="px-32 py-24 flex justify-center">
        <h2 className="font-playfair text-[42px] italic font-semibold text-[#0D2818] leading-[1.3] text-center max-w-[720px]">
          We help the world&apos;s largest travel companies solve big problems and unlock new opportunities.
        </h2>
      </section>

      {/* Portfolio Logos */}
      <section className="bg-white px-14 py-12 flex flex-col items-center gap-12">
        {[0, 1].map((row) => (
          <div key={row} className="flex gap-10 justify-between w-[1008px]">
            {[0, 1, 2, 3, 4, 5].map((col) => (
      ...
      <section className="bg-white px-14 py-20 flex flex-col items-center gap-12 border-y border-[#E5E5E5]">
        <span className="text-[#00D776] font-semibold text-xs tracking-[2px]">INVESTMENT FOCUS</span>
        <h2 className="font-playfair text-4xl italic text-[#1A2E23]">Our Investment Categories</h2>
        <div className="flex gap-5 w-full">
          {categories.map((c, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-4 p-6 border border-[#E5E5E5]">
      ...
      {/* Platform Section */}
      <section className="bg-white px-14 py-24 flex flex-col gap-10">
        <h2 className="font-playfair text-[42px] italic font-semibold text-[#0D2818] leading-[1.2] max-w-[800px]">We&apos;ve built a different kind of platform.</h2>
        ...
        <div className="w-full h-[400px] bg-cover bg-center rounded" style={{ backgroundImage: "url('/images/modern-platform.png')" }} />
      </section>

      {/* Backed By */}
      <section className="bg-white px-14 py-40 flex flex-col items-center gap-12">
        <h2 className="font-playfair text-4xl italic font-semibold text-[#0D2818] leading-[1.3] text-center max-w-[800px]">We are backed by visionary corporations from every corner of the travel world.</h2>
        {[0, 1].map((row) => (
          <div key={row} className="flex gap-12 justify-between w-[1008px]">
            {[0, 1, 2, 3].map((col) => (
              <LogoPlaceholder key={col} />
            ))}
          </div>
