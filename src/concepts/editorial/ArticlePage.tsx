          ...
          <div className="flex flex-col gap-4">
            {stats.map((s, i) => (
              <div key={i} className="border p-5 flex items-center justify-between" style={{ borderColor: c.rule }}>
                ...
                <span className="text-[0.65rem] uppercase tracking-[0.18em]" style={{ ...sans, color: c.muted, fontWeight: c.sansWeight }}>{s.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 md:px-12 py-16 md:py-20">
        <div className="max-w-5xl mx-auto">
          <div className="aspect-[21/9] border flex items-center justify-center" style={{ backgroundColor: c.surface, borderColor: c.rule }}>
            <span className="text-[0.7rem] uppercase tracking-[0.2em]" style={{ ...sans, color: c.muted, opacity: 0.4 }}>Article Image</span>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 md:px-12 pb-24 md:pb-32">
        <div className="max-w-3xl mx-auto">
          ...
          <blockquote className="border-l-2 pl-8 my-12" style={{ borderColor: c.accent }}>
            <p className="text-[1.3rem] leading-[1.65] font-light italic" style={{ ...serif, color: c.bodyText, fontWeight: c.bodyWeight }}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col">
            {related.map((r, i) => (
              ...
              >
                <span className="text-[0.68rem] uppercase tracking-[0.16em] shrink-0 w-24" style={{ ...sans, color: c.muted, fontWeight: c.sansWeight }}>{r.date}</span>
                <span className="text-[1.25rem] md:text-[1.25rem] font-light italic flex-1 group-hover:text-[#C49A45] transition-colors" style={{ ...serif, color: c.text, fontWeight: c.headingWeight }}>{r.title}</span>
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <Link href="/news" className="group flex items-center gap-4 hover:opacity-80 transition-colors" style={{ color: c.muted }}>
            ...
            <div>
              ...
              <span className="text-[1rem] font-light italic" style={{ ...serif, fontWeight: c.headingWeight }}>Portfolio Company StaySync Closes $28M Series A</span>
            </div>
          </Link>
          <Link href="/news" className="group flex items-center gap-4 hover:opacity-80 transition-colors text-right" style={{ color: c.muted }}>
            <div>
              ...
              <span className="text-[1rem] font-light italic" style={{ ...serif, fontWeight: c.headingWeight }}>Thayer Fund III Announces Final Close at $150M</span>
            </div>
            <span>&rarr;</span>
          </Link>
        </div>
