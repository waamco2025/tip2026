    ...
    <div className="min-h-screen transition-colors duration-500" style={{ backgroundColor: c.bg, color: c.text }}>
      ...
      <section className="px-6 md:px-12 py-24 md:py-40">
        <div className="max-w-7xl mx-auto">
          <span className="text-[0.62rem] uppercase tracking-[0.22em] block mb-8" style={{ ...sans, color: c.accent }}>Portfolio</span>
          ...
          <div className="flex flex-wrap gap-8 md:gap-16">
            {metaStats.map((s, i) => (
              <div key={i}>
                <span className="text-[1.6rem] font-light block mb-1" style={{ ...serif, color: c.accent }}>{s.value}</span>
                <span className="text-[0.58rem] uppercase tracking-[0.18em]" style={{ ...sans, color: c.muted }}>{s.label}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {filtered.map((co, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="aspect-[4/3] border mb-5 flex items-end p-5 group-hover:border-[#C49A45]/30 transition-colors" style={{ backgroundColor: c.surface, borderColor: c.rule }}>
                  <span className="text-[0.5rem] uppercase tracking-[0.15em]" style={{ ...sans, color: c.muted, opacity: 0.4 }}>Company Image</span>
                </div>
                <span className="text-[0.55rem] uppercase tracking-[0.2em] block mb-2" style={{ ...sans, color: c.accent }}>{co.category}</span>
                <h3 className="text-[1.3rem] font-light italic mb-2 group-hover:text-[#C49A45] transition-colors" style={{ ...serif, color: c.text }}>{co.name}</h3>
                <p className="text-[0.82rem] leading-[1.7]" style={{ ...sans, color: c.muted }}>{co.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-12 md:gap-20">
            <div>
              ...
              <span className="text-[0.62rem] uppercase tracking-[0.18em] cursor-pointer hover:opacity-80 transition-colors" style={{ ...sans, color: c.accent }}>
            ...
            <div className="flex flex-col gap-6">
              {featuredStats.map((s, i) => (
                <div key={i} className="border p-6 flex items-center justify-between" style={{ borderColor: c.rule }}>
                  <span className="text-[2rem] font-light" style={{ ...serif, color: c.accent }}>{s.value}</span>
                  <span className="text-[0.6rem] uppercase tracking-[0.18em]" style={{ ...sans, color: c.muted }}>{s.label}</span>
                </div>
              ))}
            </div>
          </div>
